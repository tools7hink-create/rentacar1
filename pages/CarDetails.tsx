        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="space-y-4">
            <div className="aspect-[4/3] rounded-3xl overflow-hidden shadow-lg relative bg-slate-100">
               <img src={images[activeImage] || car.imageUrl} alt={car.name} className={`w-full h-full object-cover transition-all duration-500 ${!car.available ? 'grayscale' : ''}`} />
               {!car.available && (
            ...
            {/* Horizontal Scrollable Gallery Thumbnails */}
            {images.length > 1 && (
                ...
                {images.map((img, idx) => (
                    ...
                    className={`flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 transition-all snap-start ${activeImage === idx ? 'border-luxury-black ring-2 ring-luxury-black/20 opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}
                    >
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sticky bottom-6 bg-white/80 backdrop-blur-md p-4 sm:p-0 sm:static rounded-2xl border border-slate-200 sm:border-none shadow-xl sm:shadow-none z-10">
              ...
              <Button variant="outline" size="lg" className="px-6" onClick={() => setIsBookingModalOpen(true)}>
